...
def main():
    ...
    env = retro.make(game=args.game, state=args.state, use_restricted_actions=retro.Actions.ALL, scenario=args.scenario, players=args.players)
    obs = env.reset()
    ...
    while not win.has_exit:
        ...
        #act = [0] * 12
        #act.extend(env.action_space.sample())
        sample_act = env.action_space.sample()
        sample_act[3] = 0    # pause(start 버튼) 방지
        sample_act[15] = 0    # pause(start 버튼) 방지
        sample_act[2] = 0    # mode(start 버튼) 방지
        sample_act[14] = 0    # mode(start 버튼) 방지
        sample_act[:12] = [0]*12
        #print(act)
        #env.step(act)
        print(action)
        sample_act[:12] = action[:12]
        print(sample_act)
        ...
        obs, rew, done, info = env.step(sample_act)
        recorded_actions.append(sample_act)
        steps += 1
